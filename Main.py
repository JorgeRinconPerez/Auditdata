...
def explore(df):
  ...
  y = pd.DataFrame(columns=["Column Name", "Type", "Cardinality", "Granularity", "% of Nulls", "Examples"], dtype=object)
  y["Cardinality"] = pd.to_numeric(y["Cardinality"])
  y["% of Nulls"] = pd.to_numeric(y["% of Nulls"])
  longitud = len(df)
  for i in df.columns:
    ...
    x = pd.DataFrame({'Column Name': [nombre_variable],
                      ...
                      "% of Nulls": [round(n_nulo_variable*100, 2)],
